  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)] -mt-4 animate-in fade-in duration-500">
            ...
            <div className="p-8 border-b border-gray-100 bg-[#131926] text-white shrink-0 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-5 mb-2">
                        ...
                        <h2 className="text-2xl font-black uppercase tracking-tight leading-none text-white">Market Quality<br/>Disputes</h2>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-gray-50/50 custom-scrollbar">
                {issues.length === 0 ? (
                    ...
                    <div key={issue.id} 
                        ...
                        className="bg-white rounded-[2.5rem] p-10 md:p-12 shadow-sm hover:shadow-2xl transition-all cursor-pointer active:scale-[0.98] group border-2 border-red-50/50"
                    >
                        <div className="flex flex-col space-y-6">
                            <div>
                                <span className="bg-[#FFF1F2] text-[#E11D48] px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border border-red-100">Produce Dispute</span>
                                <h3 className="font-black text-gray-900 text-2xl uppercase tracking-tighter mt-6 leading-none group-hover:text-red-600 transition-colors">{getBuyerName(orders.find(o => o.id === issue.orderId)?.buyerId || '')}</h3>
                            </div>
                            <p className="text-red-900 font-bold text-xl italic leading-relaxed">{`"${issue.description}"`}</p>
                        </div>
                    </div>
                ))}
            </div>
      ...
      <DisputeDetailsModal 
        ...
        issue={selectedIssue}
        order={orders.find(o => o.id === selectedIssue?.orderId) || null}
        products={allProducts}
        users={users}
        customers={customers}
      />
